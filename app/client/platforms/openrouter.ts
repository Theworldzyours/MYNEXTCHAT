"use client";
import { ApiPath, OpenRouter, OPENROUTER_BASE_URL } from "@/app/constant";
import {
  useAccessStore,
  useAppConfig,
  useChatStore,
...
export class OpenRouterApi implements LLMApi {
      ...
      if (shouldStream) {
        ...
        return streamWithThink(
          ...
          (text: string, runTools: ChatMessageTool[]) => {
            ...
            return {
              text: content || "",
              finished: choices[0]?.finish_reason === "stop",
            };
          },
        );
      } else {
        ...
        options.onUpdate?.(message, { text: message, finished: true });
      }
    } catch (e) {
  ...
  async models(): Promise<LLMModel[]> {
    ...
    try {
      ...
      const resJson = await res.json();
      const chatModels =
        resJson.data?.filter((m: any) => m.type === "chat") || [];
